...
onMounted((): void => {
  ...
  window.mainApi.send('msgRequestSQLData', 'SELECT ent.ENTI_NAME, ent.ENTI_LOCKED, cat.ENCA_NAME FROM Entities ent INNER JOIN Entity_Category cat on (ent.ENTI_CATEGORY_ID = cat.ENCA_ID) where ENTI_DB_ID = 0')
  ...
  <BasicApp>
    ...
    <v-container class="white">
      <v-row class="mb-3" height="50">
        <!--Top Left><-->
        <v-col cols="10">
          <v-breadcrumbs :items="['DATABASE NAME', 'V.x.y', 'DERIVED FROM', 'DATABASE NAME', 'V.y.12']">
          </v-breadcrumbs>
        </v-col>
        <!--Top Right><-->
        <v-col cols="2">
          ...
          <v-btn density="comfortable" icon="mdi-content-duplicate" :ripple="true">
            <v-icon color="black"></v-icon>
            <v-tooltip activator="parent" location="top">Clone</v-tooltip>
          </v-btn>
          <v-btn density="comfortable" icon="mdi-exit-to-app" :ripple="true">
            <v-icon color="black"></v-icon>
            <v-tooltip activator="parent" location="top">Exit to Home</v-tooltip>
          </v-btn>
        </v-col>
      </v-row>
      <v-row class="mb-3" no-gutters>
        <v-col cols="5" class="bg-red">
          Database Content -> {{ $t(titleKey) }} -> {{  titleKey }}
          <!-- here the subcontainer begins -->
          <v-container>
            <v-row style="min-height: 50px;" class="bg-indigo">
              <v-col cols="12">
                Entries in Database {{ $t("generaltext.hello") }}
                {{  sqlResult }}
              </v-col>
            </v-row>
            <v-row style="min-height: 550px;" class="bg-orange">
              <v-col cols="12">
                <v-table density="compact">
                  ...
                  <tbody>
                    <tr v-for="(item, index) in sqlResult" :key="index">
                      <td> {{ index }}</td>
                      <td> {{ item.ENTI_NAME }} .. {{ item.ENCA_NAME }}</td>
                      <td> 
                        <v-checkbox 
                          v-model.number="item.ENTI_LOCKED"
                          :label="`${item.ENTI_LOCKED}`"
                          :true-value="1"
                          :false-value="0"
                        ></v-checkbox>
                      </td>
                      ...
                      <td> <v-btn density="compact" icon="mdi-trash-can-outline" variant="tonal"></v-btn> </td>
                    </tr>
                    <!--tr>
                      <td> #1 </td>
                      <td> <v-btn density="compact" icon="mdi-file-edit-outline" variant="tonal"></v-btn> </td>
            ...
            <v-row style="min-height: 75px;" class="bg-indigo">
              <v-col cols="12">
                <v-btn density="comfortable" icon="mdi-table-plus" variant="tonal">
                  <v-icon color="black"></v-icon>
                  <v-tooltip activator="parent" location="top">Add entry</v-tooltip>
                </v-btn>
                <v-btn density="comfortable" icon="mdi-file-export" variant="tonal">
                  <v-icon color="black"></v-icon>
                  <v-tooltip activator="parent" location="top">New database from selection</v-tooltip>
                </v-btn>
              </v-col>
            </v-row>
          </v-container>
        ...
        <v-col cols="3" class="bg-green">
          ...
          <!--v-container>
                ...
                <v-table density="compact">
                  ...
                  <tbody>
                    <tr>
                      ...
                      <td> <v-text-field density="compact" label="Label">22</v-text-field></td>
                    </tr>
                    <tr>
            ...
            <v-row style="max-height: 75px;" class="bg-indigo">
              <v-col cols="12">
                <v-textarea
                  :model-value="value"
                  label="Notes"
                  ...
                  <v-table density="compact">
                    ...
                    <tbody>
                      <tr>
                        <td> Scenario 1 </td>
                        <td> 19.04.2024</td>
                        <td> <v-btn density="compact" icon="mdi-update" variant="tonal"></v-btn> </td>
                      </tr>
                      <tr>
                        <td> Scenario 2 </td>
                        <td> 21.02.2024</td>
